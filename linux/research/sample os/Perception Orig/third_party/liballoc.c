...
#define ALIGN( ptr )													\
		...
		{																\
			size_t diff;												\
			ptr = (void*)((size_t)ptr + ALIGN_INFO);					\
			diff = (size_t)ptr & (ALIGNMENT-1);						\
			if ( diff != 0 )											\
			{															\
				diff = ALIGNMENT - diff;								\
				ptr = (void*)((size_t)ptr + diff);					\
			}															\
			*((ALIGN_TYPE*)((size_t)ptr - ALIGN_INFO)) = 			\
				diff + ALIGN_INFO;										\
		}															
...
#define UNALIGN( ptr )													\
		if ( ALIGNMENT > 1 )											\
		{																\
			size_t diff = *((ALIGN_TYPE*)((size_t)ptr - ALIGN_INFO));	\
...
void *PREFIX(imalloc)(size_t req_size)
{
	...
	while ( maj != NULL )
	{
		...
		if ( maj->first == NULL )
		{
			maj->first = (struct liballoc_minor*)((size_t)maj + sizeof(struct liballoc_major) );
			...
			p = (void*)((size_t)(maj->first) + sizeof( struct liballoc_minor ));
		...
		if ( diff >= (size + sizeof(struct liballoc_minor)) )
		{
			...
			maj->first->prev = (struct liballoc_minor*)((size_t)maj + sizeof(struct liballoc_major) );
		...
		while ( min != NULL )
		{
				...
				if ( min->next == NULL )
				{
					...
					{
						// yay....
						min->next = (struct liballoc_minor*)((size_t)min + sizeof( struct liballoc_minor ) + min->size);
						min->next->prev = min;
						min = min->next;
						min->next = NULL;
						...
						p = (void*)((size_t)min + sizeof( struct liballoc_minor ));
						ALIGN( p );
				...
				if ( min->next != NULL )
				{
					...
					{
						// yay......
						new_min = (struct liballoc_minor*)((size_t)min + sizeof( struct liballoc_minor ) + min->size);
						...
						p = (void*)((size_t)new_min + sizeof( struct liballoc_minor ));
						ALIGN( p );
