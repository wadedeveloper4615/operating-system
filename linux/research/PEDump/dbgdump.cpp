...
void DumpImageDbgHeader(PIMAGE_SEPARATE_DEBUG_HEADER pImageSepDbgHeader) {
    ...
    printf("  %-*s%08X -> %s", headerFieldWidth, "TimeDateStamp:", pImageSepDbgHeader->TimeDateStamp, ctime((time_t*) &pImageSepDbgHeader->TimeDateStamp));
...
void DumpDbgFile(PIMAGE_SEPARATE_DEBUG_HEADER pImageSepDbgHeader) {
    ...
    DumpSectionTable((PIMAGE_SECTION_HEADER) (pImageSepDbgHeader + 1), pImageSepDbgHeader->NumberOfSections, TRUE);

    DumpDebugDirectory(MakePtr(PIMAGE_DEBUG_DIRECTORY, pImageSepDbgHeader, sizeof(IMAGE_SEPARATE_DEBUG_HEADER) + (pImageSepDbgHeader->NumberOfSections * sizeof(IMAGE_SECTION_HEADER)) + pImageSepDbgHeader->ExportedNamesSize), pImageSepDbgHeader->DebugDirectorySize, (ULONGLONG) pImageSepDbgHeader);
    ...
    if (g_pCOFFHeader) {
        ...
        g_pCOFFSymbolTable = new COFFSymbolTable(MakePtr(PVOID, g_pCOFFHeader, g_pCOFFHeader->LvaToFirstSymbol), g_pCOFFHeader->NumberOfSymbols);
