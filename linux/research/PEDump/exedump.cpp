...
void DumpImportsSection(DWORD base, PIMAGE_NT_HEADERS pNTHeader) {
    ...
    while (1) {
        ...
        // mem mapped file.
        thunk = (PIMAGE_THUNK_DATA) GetPtrFromRVA((ULONGLONG) thunk, pNTHeader, base);
        if (!thunk)
            return;

        thunkIAT = (PIMAGE_THUNK_DATA) GetPtrFromRVA((ULONGLONG) thunkIAT, pNTHeader, base);
        ...
        while (1) // Loop forever (or until we break out)
        {
            ...
            if (thunk->u1.Ordinal & IMAGE_ORDINAL_FLAG) {
                ...
                pOrdinalName = (PIMAGE_IMPORT_BY_NAME) GetPtrFromRVA((ULONGLONG) pOrdinalName, pNTHeader, base);
...
void DumpExportsSection(DWORD base, PIMAGE_NT_HEADERS pNTHeader) {
    ...
    printf("  TimeDateStamp:   %08X -> %s", exportDir->TimeDateStamp, ctime((time_t*) &exportDir->TimeDateStamp));
...
void DumpRuntimeFunctions(DWORD base, PIMAGE_NT_HEADERS pNTHeader) {
    ...
    DWORD cEntries =
    GetImgDirEntrySize( pNTHeader, IMAGE_DIRECTORY_ENTRY_EXCEPTION ) / sizeof(IMAGE_RUNTIME_FUNCTION_ENTRY);
    if (0 == cEntries)
        return;
    ...
    for (unsigned i = 0; i < cEntries; i++, pRTFn++) {
        ...
        if (g_pCOFFSymbolTable) {
            PCOFFSymbol pSymbol = g_pCOFFSymbolTable->GetNearestSymbolFromRVA(pRTFn->BeginAddress - pNTHeader->OptionalHeader.ImageBase,
            TRUE);
            if (pSymbol)
...
char *SzRelocTypes[] = { "ABSOLUTE", "HIGH", "LOW", "HIGHLOW", "HIGHADJ", "MIPS_JMPADDR", "SECTION", "REL32" };
...
void DumpBaseRelocationsSection(DWORD base, PIMAGE_NT_HEADERS pNTHeader) {
    ...
    while (baseReloc->SizeOfBlock != 0) {
        ...
        for (i = 0; i < cEntries; i++) {
            ...
            printf("  %08X %s", (*pEntry & 0x0FFF) + baseReloc->VirtualAddress, szRelocType);
...
void DumpBoundImportDescriptors(DWORD base, PIMAGE_NT_HEADERS pNTHeader) {
    ...
    while (pibid->TimeDateStamp) {
        ...
        printf("  %-12s  %08X -> %s", base + bidRVA + pibid->OffsetModuleName, pibid->TimeDateStamp, ctime((time_t*) &pibid->TimeDateStamp));
        ...
        for (i = 0; i < pibid->NumberOfModuleForwarderRefs; i++) {
            printf("    forwarder:  %-12s  %08X -> %s", base + bidRVA + pibfr->OffsetModuleName, pibfr->TimeDateStamp, ctime((time_t*) &pibfr->TimeDateStamp));
...
void DumpExeFile(PIMAGE_DOS_HEADER dosHeader) {
    ...
    {						// through the debug directory?
        g_pCOFFSymbolTable = new COFFSymbolTable((PVOID) (base + pNTHeader->FileHeader.PointerToSymbolTable), pNTHeader->FileHeader.NumberOfSymbols);
    }
