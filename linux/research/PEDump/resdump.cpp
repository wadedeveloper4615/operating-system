...
char *SzResourceTypes[] = { "???_0", "CURSOR", "BITMAP", "ICON", "MENU", "DIALOG", "STRING", "FONTDIR", "FONT", "ACCELERATORS", "RCDATA", "MESSAGETABLE", "GROUP_CURSOR", "???_13", "GROUP_ICON", "???_15", "VERSION", "DLGINCLUDE", "???_18", "PLUGPLAY", "VXD", "ANICURSOR", "ANIICON" };
...
DWORD GetOffsetToDataFromResEntry(DWORD base, DWORD resourceBase, PIMAGE_RESOURCE_DIRECTORY_ENTRY pResEntry) {
    ...
    PIMAGE_RESOURCE_DATA_ENTRY pResDataEntry = (PIMAGE_RESOURCE_DATA_ENTRY) (resourceBase + pResDirEntry->OffsetToData);

    return pResDataEntry->OffsetToData;
}

void DumpStringTable(DWORD base, PIMAGE_NT_HEADERS pNTHeader, DWORD resourceBase, PIMAGE_RESOURCE_DIRECTORY_ENTRY pStrResEntry, DWORD cStrResEntries) {
...
void DumpDialogs(DWORD base, PIMAGE_NT_HEADERS pNTHeader, DWORD resourceBase, PIMAGE_RESOURCE_DIRECTORY_ENTRY pDlgResEntry, DWORD cDlgResEntries) {
    for (unsigned i = 0; i < cDlgResEntries; i++, pDlgResEntry++) {
        ...
        if ( HIWORD(*pDlgStyle) != 0xFFFF) {
            ...
            printf("  controls: %u\n", pDlgTemplate->cdit);
            printf("  (%u,%u) - (%u,%u)\n", pDlgTemplate->x, pDlgTemplate->y, pDlgTemplate->x + pDlgTemplate->cx, pDlgTemplate->y + pDlgTemplate->cy);
            ...
            for (unsigned i = 0; i < pDlgTemplate->cdit; i++) {
                // Control item header....
                pDlgItemTemplate = (DLGITEMTEMPLATE*) (((ULONGLONG) pDlgItemTemplate + 3) & ~3);

                printf("    style: %08X\n", pDlgItemTemplate->style);
                printf("    extended style: %08X\n", pDlgItemTemplate->dwExtendedStyle);

                printf("    (%u,%u) - (%u,%u)\n", pDlgItemTemplate->x, pDlgItemTemplate->y, pDlgItemTemplate->x + pDlgItemTemplate->cx, pDlgItemTemplate->y + pDlgItemTemplate->cy);
                printf("    id: %u\n", pDlgItemTemplate->id);
                ...
                PBYTE pCreationData = (PBYTE) (((ULONGLONG) pTitle + 1) & 0xFFFFFFFE);
...
void DumpResourceEntry(PIMAGE_RESOURCE_DIRECTORY_ENTRY resDirEntry, DWORD resourceBase, DWORD level) {
    ...
    if (resDirEntry->OffsetToData & IMAGE_RESOURCE_DATA_IS_DIRECTORY) {
        DumpResourceDirectory((PIMAGE_RESOURCE_DIRECTORY) ((resDirEntry->OffsetToData & 0x7FFFFFFF) + resourceBase), resourceBase, level, resDirEntry->Name);
        return;
    }
    ...
    printf("DataRVA: %05X  DataSize: %05X  CodePage: %X\n", pResDataEntry->OffsetToData, pResDataEntry->Size, pResDataEntry->CodePage);
}
...
void DumpResourceDirectory(PIMAGE_RESOURCE_DIRECTORY resDir, DWORD resourceBase, DWORD level, DWORD resourceType) {
    ...
    printf("ResDir (%s) Entries:%02X (Named:%02X, ID:%02X) TimeDate:%08X", szType, resDir->NumberOfNamedEntries + resDir->NumberOfIdEntries, resDir->NumberOfNamedEntries, resDir->NumberOfIdEntries, resDir->TimeDateStamp);
    ...
    if (level == 1 && (resourceType == (ULONGLONG) RT_STRING)) {
        pStrResEntries = resDirEntry;
        cStrResEntries = resDir->NumberOfIdEntries;
    }

    // If it's a stringtable, save off info for future use
    if (level == 1 && (resourceType == (ULONGLONG) RT_DIALOG)) {
...
void DumpResourceSection(DWORD base, PIMAGE_NT_HEADERS pNTHeader) {
    ...
    DumpResourceDirectory(resDir, (ULONGLONG) resDir, 0, 0);
    ...
    if (cStrResEntries) {
        ...
        DumpStringTable(base, pNTHeader, (ULONGLONG) resDir, pStrResEntries, cStrResEntries);
        printf("\n");
    }
    ...
    if (cDlgResEntries) {
        ...
        DumpDialogs(base, pNTHeader, (ULONGLONG) resDir, pDlgResEntries, cDlgResEntries);
        printf("\n");
    }
}
