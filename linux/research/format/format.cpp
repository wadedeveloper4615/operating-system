...
int format_volume(char vol, FormatParamsPtr params) {
    ...
    FAT32BootSectorPtr pFAT32BootSect;
    FAT32FSInfoPtr pFAT32FsInfo;
    ...
    // open the drive
    hDevice = CreateFile(DriveDevicePath,
    ...
    bRet = DeviceIoControl((HANDLE) hDevice,              // handle to device
    ...
    IOCTL_DISK_GET_PARTITION_INFO, NULL, 0, &piDrive, sizeof(piDrive), &cbRet, NULL);

    if (!bRet) {
        printf("IOCTL_DISK_GET_PARTITION_INFO failed, trying IOCTL_DISK_GET_PARTITION_INFO_EX\n");
        bRet = DeviceIoControl(hDevice,
        IOCTL_DISK_GET_PARTITION_INFO_EX, NULL, 0, &xpiDrive, sizeof(xpiDrive), &cbRet, NULL);
    ...
    pFAT32BootSect = (FAT32BootSectorPtr) VirtualAlloc(NULL, BytesPerSect, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
    pFAT32FsInfo = (FAT32FSInfoPtr) VirtualAlloc(NULL, BytesPerSect, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
    ...
    if (!bGPTMode) {
        ...
        bRet = DeviceIoControl(hDevice,
        IOCTL_DISK_SET_PARTITION_INFO, &spiDrive, sizeof(spiDrive), NULL, 0, &cbRet, NULL);
